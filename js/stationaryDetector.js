...
The macroscopic output also aggregates over the whole road while the 
microscopic output also gives the lane index of the passage event[CHECK]
...
function stationaryDetector(road,u,dtAggr){
  ...
  this.exportString=""; // for export to file
    ...
    this.vehCount=0; // counting inside each aggregation interval (whole road)
...
stationaryDetector.prototype.updateExportString=function(){
  ...
  if(rest<dt-0.0001){
    ...
    var speedStr=((this.historyFlow[this.iAggr]>0)
		  ? 3.6*this.historySpeed[this.iAggr].toFixed(1)
		  : "--");
...
stationaryDetector.prototype.display=function(textsize){
  // console.log("Detector display: scale=",scale);
  ...
  var dr=2.5; // about wLane but this.road.wLane not always defined
  var roadWidth=this.road.widthLeft(this.u)+this.road.widthRight(this.u)
  var toRight_axis=-0.5*roadWidth-(1+Math.abs(sphi))*dr;
  ...
  if(false){
    console.log("roadWidth=",roadWidth," this.u=",this.u,
		" toRight_axis=",toRight_axis,
		" cphi=",cphi," phi=",phi," scale=",scale,
		" this.road.get_xPix(this.u, 0,scale)=",
		this.road.get_xPix(this.u, 0,scale),
		" this.road.get_phi(this.u)=",
		this.road.get_phi(this.u),
		"");
  }
    // the detector line
    ...
    var detLineDist=1;     // dist of the loops of double-loop detector [m]
  var detLineLength=roadWidth;

  var xCenterPix1=  scale*this.road.axis_x(this.u-0.5*detLineDist);
  var yCenterPix1= -scale*this.road.axis_y(this.u-0.5*detLineDist);//minus!!
  var xCenterPix2=  scale*this.road.axis_x(this.u+0.5*detLineDist);
  var yCenterPix2= -scale*this.road.axis_y(this.u+0.5*detLineDist); 
