...
##############################################################
set out "../figs/sim_lanes_IAM_histograms.eps"
print "plotting ../figs/sim_lanes_IAM_histograms.eps"
##############################################################
...
plot\
  "sim_lanes_IAM.hist" u 1:2 t "Motorcycles"\
     w boxes lc rgb "#aa4477ff" lw 2 fs solid 0.50,\
  "sim_lanes_IAM.hist" u 1:3 t "Cars and trucks"\
     w boxes lc rgb "#44000000" lw 3 fs transparent

##############################################################
set out "../figs/sim_lanes_IAM_x300_fund.eps"
print "plotting ../figs/sim_lanes_IAM_x300_fund.eps"
##############################################################

set key box top right

set xlabel "Density Q/V [veh./km]"
set xrange [0:700]  # strange DOS with [0:]
set yrange [0:7000]
set ylabel "Flow [veh./h]"

#rescaling by factor fact_dens (wrong lengths) //!!!
fact_dens=0.9
fact_flow=1.0
plot\
  "sim_lanes_IAM_x300.fund"\
  u (($3==0) ? 0 : fact_dens*$2/$3) : (fact_flow*$2) t "x=300 m" w p ls 1

##############################################################
set out "../figs/sim_lanes_IAM_x300_fundEdie.eps"
print "plotting ../figs/sim_lanes_IAM_x300_fundEdie.eps"
##############################################################
#rescaling by fact_dens (wrong lengths)
plot\
  "sim_lanes_IAM_x300.fundEdie"\
  u (fact_dens*$2) : (fact_flow*$3) t "x=300 m" w p ls 1




##############################################################
set out "../figs/sim_lanes_IAM_xy.eps"
print "plotting ../figs/sim_lanes_IAM_xy.eps"
##############################################################
...
set yrange [-4.5:5.5]
plot[t=0:1]\
  "sim_lanes_IAM.traj" u (filterData($2,0)*$3):($4)\
    t "" w p ls 7 ps 0.3,\
    t,t t "Motorcycles" w l ls 17,\
  "sim_lanes_IAM.traj" u (filterDataInverse($2,0)*$3):($4)\
    t "" w p ls 1 ps 0.3,\
...
##############################################################
set out "../figs/sim_lanes_IAM_xt_laneMiddle.eps"
print "plotting ../figs/sim_lanes_IAM_xt_laneMiddle.eps"
##############################################################
